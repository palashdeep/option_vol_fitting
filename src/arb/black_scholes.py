...
def bs_price(option_type, S, K, T, r, q=0, sigma=0.05):
...
def implied_vol_from_price(option_type, price, S, K, T, r, q=0, sigma_bounds=(1e-6, 5.0)):
    """Invert Black-Scholes price -> implied vol using root-finding (Brent). Returns np.nan if no solution."""
    # price must be between intrinsic and forward bounds
    # lower bound intrinsic:
    ...
    if price < intrinsic - 1e-12 or price <= intrinsic + 1e-3:
        return np.nan
    # If price is extremely close to theoretical zero, return very small vol
    try:
        def objective(sigma):
            return bs_price(option_type, S, K, T, r, q, sigma) - price
        # careful with brackets: ensure function changes sign in the interval
        a, b = sigma_bounds
        fa, fb = objective(a), objective(b)
        # If objective(a) and objective(b) have same sign, try to expand b
        if fa * fb > 0:
            # attempt a quick check: if price is larger than b-vol price, increase b
